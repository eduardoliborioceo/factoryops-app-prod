...
def listar(data_inicial: str, data_final: str, setor: str = "", linha: str = "", turno: str = "") -> list:
    return repo.listar_mes(data_inicial, data_final, setor, linha, turno)


def totais(data_inicial: str, data_final: str, setor: str = "", linha: str = "", turno: str = "") -> dict:
    return repo.totais_mes(data_inicial, data_final, setor, linha, turno)
    ...
    try:
        with app.app_context():
            for i in range(0, total, BATCH):
                resultado = repo.importar_registros_mes(registros[i:i + BATCH])
